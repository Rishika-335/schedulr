...
import {
  ChevronLeft, ChevronRight, Clock, MapPin, Globe,
  Calendar, User, Mail, FileText, ArrowLeft, Loader2
} from 'lucide-react'
import {
...
import { eventTypesApi, availabilityApi, bookingsApi, userApi } from '@/services/api'
...
export default function BookingPage() {
  ...
  const fetchSlots = useCallback(async (date) => {
    ...
    try {
      ...
      const data = await availabilityApi.getSlots(username, slug, dateStr, timezone)
      setSlots(data.slots || [])
    } catch (e) {
        ...
        <div className="grid lg:grid-cols-[320px_1fr] gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 h-fit">
            ...
            {eventType.description && (
              <p className="text-sm text-gray-500 mt-1 leading-relaxed">{eventType.description}</p>
            )}
            ...
            <div className="mt-5 pt-4 border-t border-gray-100">
              ...
              <select
                value={timezone}
                onChange={e => { setTimezone(e.target.value); if (selectedDate) fetchSlots(selectedDate) }}
          ...
          <div className="bg-white rounded-2xl border border-gray-100">
            {!showForm ? (
                        ...
                        <div key={day.toISOString()} className="flex justify-center">
                          <button
                            ...
                            className={clsx(
                              ...
                              !inMonth && 'invisible',
                              selected && 'bg-[#0069ff] text-white font-semibold shadow-md',
                              !selected && available && 'font-semibold text-gray-900 hover:bg-blue-50 hover:text-[#0069ff] cursor-pointer',
                              !selected && !available && 'text-gray-300 cursor-not-allowed',
                              today && !selected && 'ring-2 ring-[#0069ff] ring-offset-1',
                            )}
                          >
                            {format(day, 'd')}
                          </button>
              ...
              <div className="p-6 max-w-md">
                ...
                <h2 className="text-lg font-bold text-gray-900 mb-1">Enter your details</h2>
                <p className="text-sm text-gray-500 mb-6">
                  {format(selectedDate, 'EEEE, MMMM d')} at{' '}
                  {format(parseISO(selectedSlot.start_time), 'h:mm a')} · {durationLabel(eventType.duration_minutes)}
                </p>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-gray-400">
                    By scheduling, you confirm that you agree to receive a confirmation email at the address provided.
                  </p>
